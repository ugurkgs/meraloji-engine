// server.js - MERALOJİ v41.0 (DATA STREAM & LAND OPS)
...
const SPECIES_DB = {
  "levrek": { 
    ...
    advice: { EGE: { bait: "Canlı Mamun / Silikon", hook: "Circle No:1", jig: "10-15gr Jighead", depth: "0-2m (Sığ)" }, MARMARA: { bait: "Limon Rapala / Kaşık", hook: "Üçlü İğne", jig: "Hansen Kaşık", depth: "Yüzey" } },
  ...
  "lufer": { 
    ...
    advice: { EGE: { bait: "Canlı Zargana", hook: "Uzun Pala 2/0", jig: "Dalso 12cm", depth: "Orta Su" }, MARMARA: { bait: "Yaprak Zargana / Rapala", hook: "Mantarhı Takım", jig: "Ağır Kaşık", depth: "Dip/Orta" } },
  ...
  "cipura": { 
    ...
    advice: { EGE: { bait: "Canlı Mamun / Yengeç", hook: "Chinu No:2 (Kısa)", jig: "Micro Jig", depth: "Dip" }, MARMARA: { bait: "Boru Kurdu / Midye", hook: "Kısa Pala No:4", jig: "Yemli Takım", depth: "Dip" } },
  ...
  "mirmir": { 
    ...
    advice: { EGE: { bait: "Boru Kurdu / Sülünez", hook: "Uzun Pala No:6", jig: "Kokulu Silikon", depth: "Kıyı Dibi" }, MARMARA: { bait: "Boru Kurdu", hook: "İnce Tel No:5", jig: "LRF Kurt", depth: "Kıyı Dibi" } },
  ...
  "istavrit": { 
    ...
    advice: { EGE: { bait: "Tavuk / LRF Silikon", hook: "İnce No:8", jig: "2gr Jighead", depth: "Yüzey/Orta" }, MARMARA: { bait: "Çapari (Yeşil/Beyaz)", hook: "Çapari No:10", jig: "Sırtı", depth: "Değişken" } },
  ...
  "kalamar": { 
    ...
    advice: { EGE: { bait: "Kırmızı/Turuncu Zoka", hook: "Şemsiye İğne", jig: "3.0 Yamashita", depth: "Dip Üstü" }, MARMARA: { bait: "Fosforlu Zoka", hook: "Şemsiye İğne", jig: "2.5 DTD", depth: "Orta Su" } },
  ...
  "ahtapot": { 
    ...
    advice: { EGE: { bait: "Yengeç / Tavuk But", hook: "Çarpmalı Zoka", jig: "Ahtapot Zokası", depth: "Dip (Taşlık)" }, MARMARA: { bait: "Beyaz Yapay Yengeç", hook: "Çarpmalı", jig: "Plastik Yengeç", depth: "Dip (Kayalık)" } },
...
app.get('/api/forecast', async (req, res) => {
    try {
        ...
        const cacheKey = `forecast_v41_0_${lat}_${lon}`;

        // Backend beklemesini kaldırdık, animasyon frontend'de olacak
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // Kara ise Marine verilerini sahte/boş doldur
            ...
            let tacticText = isLand ? "Burası kara parçası veya sığlık. Balıkçılık verisi hesaplanamıyor. Yemci arayabilirsiniz." : "";
            if (!isLand) {
                if (weatherSummary.includes("FIRTINA")) tacticText = "⚠️ FIRTINA ALARMI! Kıyıya yaklaşma, güvenli limanları tercih et.";
                else if (windSpeed > 35) tacticText = "Rüzgar çok sert. Sahte atışı zorlaşır (Wind Knot riski). Ağır jig (40g+) kullan.";
                else if (wave > 1.5) tacticText = "Deniz çok kaba. Levrek için pusu ortamı. Köpüklü sulara 'Su Üstü' at.";
                else if (clarity > 90) tacticText = "Su kristal gibi berrak. Balık misinayı görür. Fluorocarbon lider kullan, sessiz ol.";
                else if (tempDiff < -5) tacticText = "Hava sudan çok daha soğuk (Şok Etkisi). Balık dipte uyuşuk. Yemi 'Dead Slow' sar.";
                else {
                    const isDay = (currentHour > 6 && currentHour < 19);
                    if (isDay) {
                        if (cloud > 60) tacticText = "Hava kapalı, ışık kırılıyor. Mat/Doğal (Bone) renklerle merayı geniş tara.";
                        else tacticText = "Atmosfer stabil. Gölgelik alanları ve derin kanalları yokla. Tek noktada bekleme (Search & Destroy).";
                    } else {
                        if (moon.fraction > 0.8) tacticText = "Dolunay ışığı var. Silüet veren koyu renk (Siyah/Mor) sahteler kullan.";
                        else tacticText = "Gece karanlık. Balık kıyıya yanaşır. Fosforlu (Glow) veya sesli sahtelerle balığı çek.";
                    }
                }
            }
            ...
            forecast.push({
                ...
                airTemp: tempAir // Animasyon için eklendi
            });
        }

        const responseData = { version: "v41.0 DATA STREAM", region: regionName, isLand: isLand, forecast: forecast };
...
app.listen(PORT, () => {
    console.log(`\n⚓ MERALOJİ ENGINE v41.0 (DATA STREAM) AKTİF!`);
});
